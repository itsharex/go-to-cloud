...
func buildRouters(router *gin.Engine) {
	...
	api.Use(middlewares.AuthHandler())
	{
		...
		project.GET("/:projectId/pipeline/:pipelineId/history", projects.QueryBuildPlanHistory)
		...
		project.PUT("/:projectId/deploy/:id/rollback/:historyId", projects.Rollback)
		...
		project.GET("/:projectId/artifact/:artifactId", projects.QueryArtifactItemsByProjectId)
		project.GET("/:projectId/artifact", projects.QueryArtifactsByProjectId)
